...
+ function($) {
  ...
  $.modal = function(params, onOpen) {
    ...
    var tpl = '<div class="weui-dialog">' +
                ...
                ( params.text ? '<div class="weui-dialog__bd" style="max-height: 300px;overflow: scroll;">'+params.text+'</div>' : '')+
  ...
  $.prompt = function(text, title, onOK, onCancel, input) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><textarea class="weui-input weui-prompt-input" id="weui-prompt-input" value="' + (config.value || '') + '" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
  ...
  //如果参数过多，建议通过 config 对象进行配置，而不是传入多个参数。
  $.tiren = function(text, title, onOK, onCancel, input) {
    var config;
    if (typeof text === 'object') {
      config = text;
    } else {
	...
	for(var i=0;i<config.text.length;i++){
		oplist.push('<div class="weui-cells weui-cells_checkbox">' +
      '<label class="weui-cell weui-check__label" >'+ '<div class="weui-cell__bd tc" >'+ '<p class="weui-prompt-text">'+config.text[i].Member+'</p>' +'</div>' +
      '<div class="weui-cell__hd" >' + '<input type="checkbox" class="weui-check"  name="gmember" value="' + config.text[i].OpenId + '"/>' + '<i class="weui-icon-checked" />' + '</i>' + '</div>' +
      '</label>' + '</div>')
	}
    var modal = $.modal({
      text: oplist.join(''),
      ...
      buttons: [
        ...
        },
        {
          ...
          onClick: function() {
			  var cklist=[];
			  $("input[name='gmember']:checked").each(function(){cklist.push($(this).val())});
            var input = $("#weui-prompt-input").val();
            if (!config.empty && (input === "" || input === null)) {
              modal.find('.weui-prompt-input').focus()[0].select();
              return false;
            ...
            $.closeModal();
            config.onOK && config.onOK.call(modal, cklist);
          }
        }]
    }, function () {
      //this.find('.weui-prompt-input').focus()[0].select();
